...
# Multi-factorial ANOVA for the biomass of pleuroncodes as the function of D/N, onshore/offshore and oxygen bins
...
#importing dataset
MyData <- read.csv(file="/home/sahed/Desktop/office/stats/anova/df_Pleuroncodes.csv", header=TRUE, sep="\t")
...
fligner.test(MyData$biomass~MyData$bins)                                          # low p values (varience are not homogeneous)
...
# model output (ANOVA)
summary(model1)                                                                   # significant only on MyData$on_off:MyData$D_N
capture.output(anova(model1), file="anovaTable_pleuroncodes.txt")
# post-hoc test (Tukey HSD for the pairwise to see the means which are significantly different than eac other)
TukeyHSD(model1)
capture.output(TukeyHSD(model1), file = "tukeyTable_pleuroncodes.txt")
