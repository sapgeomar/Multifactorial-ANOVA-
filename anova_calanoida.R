...
# Multi-factorial ANOVA for the biomass of calanoida as the function of D/N, onshore/offshore and oxygen bins
...
##### importing dataset #####
#myData<- read.table(file.choose(), header = ,)
MyData <- read.csv(file="/home/sahed/Desktop/office/stats/anova/df_calanoids.csv", header=TRUE, sep=",")
...
fligner.test(MyData$biomass~MyData$on_off)                                        # high p values (homogeneous varience)
fligner.test(MyData$biomass~MyData$D_N)                                           # low p values (varience are not homogeneous)
fligner.test(MyData$biomass~interaction(MyData$bins,MyData$on_off, MyData$D_N))   # high p values (homogeneous varience)

# normality of errors
hist(resid(model1))

# test for normality
shapiro.test(resid(model1))                                                       # high p value (data are normally distributed)
...
# lets just export the anova results
capture.output(anova(model1), file="anovaTable_calanoida.txt")
...
# lets just export the TukeyHSD results
capture.output(TukeyHSD(model1), file="tukeyTable_calanoida.txt")
