...
# Multi-factorial ANOVA for the biomass of eucalanoida as the function of D/N, onshore/offshore and oxygen bins
...
#importing dataset
MyData <- read.csv(file="/home/sahed/Desktop/office/stats/anova/df_eucalanoida.csv", header=TRUE, sep="\t")
...
capture.output(anova(model1), file="anovaTable_eucalanoida.txt")

# post-hoc test (Tukey HSD for the pairwise to see the means which are significantly different than eac other)
TukeyHSD(model1)
capture.output(TukeyHSD(model1), file = "tukey_table_eucalanoida.txt")
